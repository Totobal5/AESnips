/// @description Perform the snip logic

show_debug_message($"(Blue cowboy) image_index: {my_player.image_index}");
...
if (keyboard_lastkey != -1) // If a key has been pressed
{
	...
	if (keyboard_lastkey >= ord("0") && keyboard_lastkey <= ord("9"))
	{
		...
		shoot_loop.SetIterations(fire_count - 1);
		...
		shoot_loop_with_twirl.SetIterations((fire_count == 0) ? -1 : 0);
	}
